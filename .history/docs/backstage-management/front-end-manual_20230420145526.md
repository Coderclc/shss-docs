...
| $.common.uniqueFn(); | array（数组） | 从数组中删除重复项 |
| $.common.join(); | array（数组）, separator（分隔符） | 将数组中的所有元素连接成一个字符串 |
| $.common.formToJSON(); | formId（表单ID） | 获取 form 下的所有字符串并转换为 JSON 对象 |
| $.common.dictToSelect(); | datas（字典数据）, value（值）, name（属性名） | 将数据字典转换为下拉框 |
| $.common.getLength(); | obj（对象） | 获取对象长度 |
| $.common.isMobile(); | 无 | 确定是否移动 |
| $.common.numValid(); | text（值） | 验证一个数字表达式，必须是 0-9 |
| $.common.enValid(); | text（值） | 验证一个英文表达式，必须是az和AZ字母 |
| $.common.enNumValid(); | text（值） | 验证一个英文，数字表达式，必须包含（字母，数字） |
| $.common.charValid(); | text（值） | 验证一个英文、数字、特殊字符表达式，必须包含(字母、数字、特殊字符!@#$%^&*()-=+) |